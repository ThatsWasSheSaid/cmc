$(document).ready( function() {
    ...
    $("#form-addMaterial").validate({
        ...
        messages:{
            ...
            alturaMaterial:{
                required:"Informe a altura do seu Piso!",
            },
            comprimentoMaterial:{
    ...
    $("#form-addMaterial").submit(function() {
        ...
        $.ajax({
            type: "POST",
            url: 'scripts/pisos.php?action=addMaterial',
    ...
    $("#form-addPasta").submit(function() {
        ...
        $("#form-material").validate({
            ...
            messages:{
                material:{
                    required:"Escolha um tipo de Piso!",
                },
                massa:{
                    required:"Informe a espessura da Argamasa!",
                },
                comComodo1:{
                    required:"Informe as medidas do cômodo!",
                },
                comComodo2:{
                    required:"Informe as medidas do cômodo!",
                }
        ...
        $("#form-item").submit(function() {
            ...
            $.ajax({
                type: "POST",
                url: 'scripts/pisos.php?action=salvarItem',
...
function buscaMateriais(){
    ...
    $.ajax({
        url: "http://localhost/aula/cmc/scripts/pisos.php?action=get",
        dataType: "json",
        async: false,
        success: function(json){
            materiais = json;
                ...
                for(i=0;i<materiais.length;i++){
                    ...
                    $('#material-'+materiais[i].idmaterial).append('<img class="material-img" id="material-'+materiais[i].idmaterial+'-img" src="_assets/img/piso.png" style="margin-top: 15px;">');
                    $('#material-'+materiais[i].idmaterial).append('<p>'+materiais[i].descricao+'  ('+materiais[i].comprimento+'x'+materiais[i].altura+'cm)</p>');
                    ...
                    $('#material-'+materiais[i].idmaterial).append('<input type="hidden" name="alturaMaterial" id="alturaMaterial-'+materiais[i].idmaterial+'" value="'+materiais[i].altura+'">');
                }
                ...
                $('#material-cadastrar').append('<p>Cadastrar Piso</p>');
        },
        complete: function() {
            return true;
        }
...
function buscaDadosItem(idItem){
    ...
    $.ajax({
        ...
        success: function(json){
            ...
            $('#comComodo1').val(item.alturaParede);
            $('#comComodo2').val(item.comprimentoParede);
...
function calculaMaterial(){
    ...
    var alturaMaterial = $('#alturaMaterial-'+idMaterial).val();

    var espessuraMassa = $('#massa').val();
    var comComodo1 = $('#comComodo1').val();
    var comComodo2 = $('#comComodo2').val();
    

    var espessuraMassa = espessuraMassa/100;
    console.log("Massa:"+espessuraMassa);
    
    var comprimentoMaterial = comprimentoMaterial/100;
    console.log("Comprimento Material:"+comprimentoMaterial);

    var alturaMaterial = alturaMaterial/100;
    console.log("Altura Material:"+alturaMaterial);
    
    var materialPorM2 = 1/[(comprimentoMaterial+espessuraMassa)*(alturaMaterial+espessuraMassa)];
    console.log("Material por m2:"+materialPorM2);


    var paredeM2 = comComodo1*comComodo2;
    var totalMaterial = paredeM2*materialPorM2;
